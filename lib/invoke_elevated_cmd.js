...
var invokeElevatedCmd = function (address, options, done) {

  debug('invokeElevatedCmd.address %j', address)

  var args = [];
  args.unshift(JSON.stringify(address))
  args.unshift(options.maxTimeoutLen || 1000);
  args.unshift(JSON.stringify(options.fileToQuit || ''));
  args.unshift(JSON.stringify(options.tokenToQuit || ''));
  args.unshift(JSON.stringify(!!options.autoClose));
